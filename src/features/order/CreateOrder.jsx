    ...
    <div className="px-4 py-6">
      ...
      <Form method="POST">
        ...
        <div className="mb-5 flex gap-2 flex-col sm:flex-row sm:items-center relative">
         ...
         { !position.latitude && !position.longitude && <span  disabled={isLoadingAddress} className="absolute right-[3px] top-[3px] sm:right-[5px] sm:top-[5px] z-50">
...
export async function action({ request }) {
  ...
  if (!isValidPhone(order.phone))
    errors.phone =
      "Please give us your correct phone number. We might need it to reach you.";

  if (Object.keys(errors).length > 0) return errors;

  //if eveerything is okay,create new order and redirect to the order page
  const newOrder = await createOrder(order);

  //do not forget to clear the cart/do not overuser
